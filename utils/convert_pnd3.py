...
def extract_pnd3_data(pdf_source: str | Path | Any) -> pd.DataFrame:
...
def format_pnd3_export_name(df: pd.DataFrame, ext: str = "xlsx") -> str:
    """Generate export filename in format pnd3-MM-YYYY.<ext>."""
    month, year = parse_month_year(df)
    clean_ext = ext.lstrip(".")
    if month and year:
        return f"pnd3-{month}-{year}.{clean_ext}"
    return f"pnd3.{clean_ext}"
...
def save_to_excel(df: pd.DataFrame, output_target: str | Path | Any) -> None:
    ...
    with pd.ExcelWriter(output_target, engine="openpyxl") as writer:
        df.to_excel(writer, index=False, sheet_name="PND3")
        worksheet = writer.sheets["PND3"]
