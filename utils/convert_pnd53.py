...
def extract_pnd53_data(pdf_source: str | Path | Any) -> pd.DataFrame:
...
def format_pnd53_export_name(df: pd.DataFrame, ext: str = "xlsx") -> str:
    """Generate export filename in format pnd53-MM-YYYY.<ext>."""
    month, year = parse_month_year(df)
    clean_ext = ext.lstrip(".")
    if month and year:
        return f"pnd53-{month}-{year}.{clean_ext}"
    return f"pnd53.{clean_ext}"
...
def save_to_excel(df: pd.DataFrame, output_target: str | Path | Any) -> None:
    ...
    with pd.ExcelWriter(output_target, engine="openpyxl") as writer:
        df.to_excel(writer, index=False, sheet_name="PND53")
        worksheet = writer.sheets["PND53"]
        for col_idx, col_name in enumerate(df.columns, start=1):
            ...
            if col_name in ["เลขประจำตัวผู้เสียภาษีอากร", "สาขาที่"]:
                for cell in worksheet.iter_cols(min_col=col_idx, max_col=col_idx, min_row=2):
                    for c in cell:
                        if c.value is not None:
                            c.number_format = "@"
            elif col_name in ["จำนวนเงินที่จ่ายในครั้งนี้", "จำนวนเงินภาษีที่หัก และนำส่งในครั้งนี้"]:
                for cell in worksheet.iter_cols(min_col=col_idx, max_col=col_idx, min_row=2):
                    for c in cell:
                        if c.value is not None:
...
def save_to_csv(df: pd.DataFrame, output_target: str | Path | Any) -> None:
    ...
    csv_df = df.copy()
    for text_col in ["เลขประจำตัวผู้เสียภาษีอากร", "สาขาที่"]:
        if text_col in csv_df.columns:
            csv_df[text_col] = csv_df[text_col].astype(str)
